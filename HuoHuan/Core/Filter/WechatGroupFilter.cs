            ...
            if (!String.IsNullOrWhiteSpace(this.text))
            {
                ...
                using (Stream s = response.GetResponseStream())
                {
                    ...
                    using (MemoryStream ms = new MemoryStream())
                    {
                        ...
                        // 获取图片文字内容
                        var dateStr = TesseractUtil.GetImageText(BitmapConverter.ToBitmap(thresholdImg)).Replace(" ", "");
                        if (!String.IsNullOrWhiteSpace(dateStr))
                        {
                            string pattern = @"内\((.+)前\)";
                            if (Regex.Matches(dateStr, pattern).Count > 0)
                            {
                                if (DateTime.TryParse(dateStr.Substring(0, dateStr.LastIndexOf("前")).Split('(')[1], out var date))
                                {
